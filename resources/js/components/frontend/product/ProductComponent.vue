        ...
        <div class="container">
            ...
            <div class="flex items-start border-t border-gray-100 max-md:border-none">
                ...
                <div class="w-full p-4 max-md:p-0">
                    ...
                    <div v-if="categoryDescription" class="mt-8 p-4 bg-gray-100 rounded-lg">
                        <h4 class="text-xl font-bold mb-2">About this Category</h4>
    ...
    methods: {
        ...
        ancestorsAndSelf: function () {
            if (typeof this.$route.query.category !== "undefined" && this.$route.query.category !== "") {
                ...
                // This is the API call that fetches the category data
                this.$store.dispatch("frontendProductCategory/ancestorsAndSelf", this.$route.query.category)
                    .then(res => {
                        // Check if the response data is structured correctly
                        console.log("API Response for ancestorsAndSelf:", res);

                        if (res.data && res.data.data && res.data.data.length > 0) {
                            // Find the category in the response that matches the ID from the URL query
                            const currentCategory = res.data.data.find(cat => cat.slug == this.$route.query.category);
                            console.log("api description", currentCategory);

                            // If a matching category is found, set the description; otherwise, clear it
                            ...
                            // If no data is returned or the structure is unexpected, clear the description
                            this.categoryDescription = null;
                        }
                        this.loading.isActive = false;
                    })
                    .catch((err) => {
                        this.loading.isActive = false;
                        this.categoryDescription = null; // Clear description on API error
                    });
            } else {
                this.productSearchForm.category = null;
                this.categoryDescription = null; // Clear description if no category is selected
            }
            ...
            this.loading.isActive = true;
            this.$store.dispatch("frontendProduct/categoryWiseProducts", this.productSearchForm)
                .then(res => {
                    this.productPrice.range = [0, res.data.data.max_price];
                    this.maxRange = res.data.data.max_price;
                    this.loading.isActive = false;
